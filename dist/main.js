...
import { version, AVAILABLE_FORMATS, parseFormatArg, fileExists, STATUS, tryCatch, log, uniqueFilename, filenameFromUrl, } from './utils.js';
...
const commandAction = async ({ inputs, options }) => {
    let exitStatus = STATUS.success;
    const outputResult = ({ data, status }) => {
...
const thumbhashFromBuffer = async ({ buffer, format }) => {
